...
import { playSaveSuccessSound } from "@/lib/sound"
...
export function SettingsView() {
  const { merchant, theme, setTheme, updateMerchant, deleteAccount, isAdmin, allMerchants, adminImpersonateMerchant, orders = [] } = useApp()
  ...
  return (
    <div className="space-y-6 max-w-4xl pb-12 rtl text-foreground">
      
      {/* 🚀 شريط عائم يظهر للأدمن أثناء معاينة المتجر لتمكينه من العودة السريعة */}
      {isAdmin && (
        <div className="bg-amber-500 text-black px-4 py-3 rounded-2xl flex items-center justify-between text-xs font-bold shadow-md">
          <div className="flex items-center gap-2">
            <span className="bg-black/20 px-2 py-0.5 rounded text-[10px]">وضع المعاينة الإدارية</span>
            <span>أنت تستعرض إعدادات هذا المتجر بصفتك المدير العام</span>
          </div>
          <button
            type="button"
            onClick={() => router.replace("/admin")}
            className="bg-black text-amber-400 px-3.5 py-1.5 rounded-xl hover:opacity-95 transition cursor-pointer flex items-center gap-1.5"
          >
            <ArrowRight className="h-3.5 w-3.5" />
            <span>العودة إلى لوحة التحكم الرئيسية للأدمن</span>
          </button>
        </div>
      )}
      ...
      <form onSubmit={handleSave} className="space-y-6">
        ...
        <div className="flex flex-wrap items-center justify-between gap-4 pt-4 border-t border-border">
          ...
          {/* 🗑️ زر حذف الحساب الذاتي للتاجر */}
          <button
            type="button"
            onClick={() => setShowDeleteModal(true)}
            className="flex items-center gap-2 rounded-lg border border-destructive/30 bg-destructive/10 px-4 py-2 text-xs font-bold text-destructive hover:bg-destructive/20 transition-all cursor-pointer"
          >
      ...
      {/* نافذة منبثقة داخلية لتأكيد حذف الحساب برسالة لطيفة */}
      {showDeleteModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4 backdrop-blur-xs">
          <div className="w-full max-w-md rounded-3xl border border-border bg-card p-6 shadow-2xl space-y-5 rtl text-right animate-in fade-in zoom-in duration-200">
            ...
            <p className="text-xs text-muted-foreground leading-loose">
              هل أنت متأكد من رغبتك في حذف متجرك وحسابك بالكامل؟ نأسف لمغادرتك، وشكراً لاستخدامك منصة إيليت العراق.
            </p>
